...
export function getStartAndEndDate(today: Date, timezone: string) {
  const currentDateBasedOnTimeZone = formatInTimeZone(today, timezone, "yyyy-MM-dd'T'HH:mm:ss");

  const startDate = fromZonedTime(startOfDay(currentDateBasedOnTimeZone), timezone);
