...
export function CheckOutSummary({ transaction, addedCharges, setAddedCharges }: Props) {
  ...
  const balance = addedCharges.reduce((acc, charge) => acc + charge, 0) - (selected?.value ?? 0);

  return (
    <Fragment>
      <div className="flex flex-col gap-2">
        {addedCharges.map((charge, index) => {
          return <CheckOutSummaryItem key={index} label="Added Charge" value={charge} handleDelete={() => removeCharge(index)} />;
        })}
        {selected ? <CheckOutSummaryItem label={selected.listLabel} value={selected.value} handleDelete={() => setSelected(null)} /> : null}
      </div>
      <div>
              ...
              {REDEPEMTIONS.map((redepemtion) => {
                return (
                  <Button
                    ...
                    disabled={transaction.customer.currentPoints < redepemtion.pointsRequired}
                    variant="outline"
