...
import { Form, FormControl, FormField, FormItem } from '@bee/ui/components/ui/form';
...
export function EditMemberButton({ user }: Props) {
  ...
  return (
    <Dialog open={open}>
      <Button className="size-6" size="icon" variant="ghost" onClick={() => setOpen(true)}>
        <Pencil className="size-4" />
      </Button>
      <DialogContent className="bg-amber-300 border-amber-300 px-16 py-12 max-w-3xl">
        <DialogHeader>
          <DialogTitle className="text-2xl text-center">Edit Member Profile</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="grid grid-cols-3 gap-2 px-24">
            ...
            <div className="flex col-span-3 gap-4 mt-4 mx-auto">
              <Button className="w-[100px]" type="button" onClick={() => setOpen(false)}>
                Cancel
              </Button>
              <Button className="bg-white text-black hover:bg-slate-50 w-[100px]" type="submit">
                Save
              </Button>
            </div>
          </form>
